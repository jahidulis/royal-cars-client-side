...
const ProductUpdate = () => {
  ...
  //   useEffect(() => {
  //     fetch("https://immense-anchorage-45433.herokuapp.com/product")
  //       .then((res) => res.json())
  //       .then((data) => setProducts(data));
  //   }, []);
  const [flag, setFlag] = useState(null);

  const url = `https://immense-anchorage-45433.herokuapp.com/product/${id}`;
  useEffect(() => {
    function fe() {
      fetch(url)
        .then((res) => res.json())
  ...
  const handleReStock = (val) => {
    fetch(
      `https://immense-anchorage-45433.herokuapp.com/product/${id}?value=${val}`,
      {
        method: "PUT",
        headers: {
